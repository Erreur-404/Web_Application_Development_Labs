...
export default class HTTPManager {
   ...
   */
  async fetchSong (id) {
    return HTTPInterface.GET(`${this.songsBaseURL}/${id}`);
  }
  ...
  async search (query, exact) {
    return HTTPInterface.GET(`${this.searchBaseURL}?search_query=${query}&exact=${exact}`);
  }
  ...
  async getPlaylistById (id) {
    try {
      return await HTTPInterface.GET(`${this.playlistBaseURL}/${id}`);
    } catch (err) {
      window.alert(`An error has occured while fetching the playlist with id ${id}`, err);
    }
  }
