...
class SongService {
  ...
  async updateSongLike (id) {
    ...
    return this.collection.replaceOne({ id }, song);
  }
  ...
  async search (substring, exact) {
    const options = exact ? "" : "i";
    const filter = {
      $or: [{ name: { $regex: `${substring}`, $options: options } },
        { artist: { $regex: `${substring}`, $options: options } },
        { genre: { $regex: `${substring}`, $options: options } }]
    };
